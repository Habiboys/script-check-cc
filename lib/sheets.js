...
function formatCheckedAt(date = new Date()) {
  ...
  // Awalan ' memaksa Google Sheets menyimpan sebagai teks (bukan parse tanggal)
  return `'${get("day")}/${get("month")}/${get("year")} ${get("hour")}:${get("minute")}:${get("second")} WIB`;
}

function sortResults(results) {
  return [...results].sort((a, b) => {
    const kelasA = (a.student.kelas || "").toUpperCase();
    const kelasB = (b.student.kelas || "").toUpperCase();
...
async function getResultSheetInfo(sheets, spreadsheetId) {
  ...
  const configuredName = process.env.RESULT_SHEET_NAME?.trim();
  let sheet;

  if (configuredName) {
    sheet = meta.data.sheets.find((s) => s.properties.title === configuredName);
    if (!sheet) {
      const names = meta.data.sheets.map((s) => s.properties.title).join(", ");
      throw new Error(`Tab "${configuredName}" tidak ditemukan. Tab yang ada: ${names}`);
    }
  } else {
    sheet = meta.data.sheets[0];
  }
...
function resultToRow(result, checkedAt) {
  return [
    ...
    linkValue(result.student.url_fe),
    linkValue(result.student.url_be),
...
async function upsertCheckerResults(results) {
  ...
  const dataRows = sorted.map((result) => resultToRow(result, checkedAt));
  const allRows = [RESULT_HEADERS, ...dataRows];
  ...
  try {
    ...
    previousRowCount = (existing.data.values || []).length;
  } catch (err) {
  ...
  try {
    await sheets.spreadsheets.values.update({
      spreadsheetId,
      range: `'${sheetName}'!A1:O${allRows.length}`,
      valueInputOption: "USER_ENTERED",
      requestBody: { values: allRows },
    });

    if (previousRowCount > allRows.length) {
      await sheets.spreadsheets.values.clear({
        spreadsheetId,
        range: `'${sheetName}'!A${allRows.length + 1}:O${previousRowCount}`,
      });
    }
  } catch (err) {
    throw formatSheetsError(err, "menulis hasil ke RESULT spreadsheet");
  }

  const dataRowCount = dataRows.length;
  const infoAfterWrite = await getResultSheetInfo(sheets, spreadsheetId);
  await applyResultSheetFormatting(
    sheets,
    spreadsheetId,
    sheetId,
    infoAfterWrite.conditionalFormatCount,
    dataRowCount
  );

  return {
    updated: dataRows.length,
    inserted: 0,
    checkedAt: checkedAt.replace(/^'/, ""),
    sheetName,
  };
}
